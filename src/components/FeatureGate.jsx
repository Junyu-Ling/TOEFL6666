...
export default function FeatureGate({ title, allowed, onLogin, children }) {
  ...
  if (authLoading || accessLoading) {
    return (
      <section className="feature-gate" lang="zh-CN">
        <p className="feature-gate__title">{title}</p>
  ...
  if (!allowed) {
    return (
      <section className="feature-gate" lang="zh-CN">
        <p className="feature-gate__title">{title}</p>
        <p className="feature-gate__text">你已登录，但还没有{title}权限。请联系管理员开通。</p>
      </section>
    );
  }
